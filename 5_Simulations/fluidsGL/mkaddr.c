...
// 4. protocol The input string indicating the protocol being used. NULL implies tcp .
//
// RETURNS:
// 0 Success.
// -1 Bad host part.
...
int mkaddr(void *addr, int *addrlen, const char *str_addr, const char *protocol)
{
	...
	{
		// Assume a hostname
		struct hostent *hp = gethostbyname(host_part);
		if (!hp)
			return -1;
	...
	else if (isdigit(*port_part))
	{
		...
		char *cp = NULL;
		long lv = strtol(port_part, &cp, 10);
		if (cp != NULL && *cp)
			return -2;
		if ((lv < 0L) || (lv >= 32768))
			return -2;
	...
	{
		// Lookup the service:
		struct servent *sp = getservbyname( port_part, protocol);
		if (!sp)
			return -2;
